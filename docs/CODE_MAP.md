...
| [lib/types.ts](../src/lib/types.ts) | ✅ | TS 类型定义：AppStatus（5 状态）、ConfigEntry、FeedbackInput/FeedbackSubmitResult/FeedbackQueueItem、AudioInputDevice、TextMode（3模式）、TextProcessMode、HistoryItem、PreviewMode、PreviewDraft、ConfirmPreviewInput、FilterWord、ASRProvider、ServiceConfig（含 ASR/LLM/翻译/反馈通道配置）、ModelInfo、TranslateResultPayload、RewriteResultPayload、RecordingTickPayload、ToastPayload、VoiceSkill、LlmStreamingDeltaPayload、UpdateInfo、UpdateProgressPayload、HotkeyConfig（含 `DEFAULT_HOTKEY_CONFIG` 常量）。与 Rust serde 类型一一对应。已移除旧的 `LocalModel` 接口（由 `ModelInfo` 替代） |
| [lib/commands.ts](../src/lib/commands.ts) | ✅ | 封装 35 个 invoke 命令：除预览/历史/配置/词典/音频/备份/模型/技能/热键/更新命令外，新增 `submitFeedback`/`flushFeedbackQueue`/`listFeedbackQueue`。均为直接 `invoke("snake_case", args)` 的薄封装，命令名集中在顶部 `COMMANDS` 常量对象 |
...
| 文件 | 状态 | 说明 |
| :--- | :---: | :--- |
| [stores/appStore.ts](../src/stores/appStore.ts) | ✅ | Zustand `usePanelStore` 已实现完整 UI 状态：UI 模式、快捷设置、服务配置（service，含 ASR/LLM/翻译/反馈通道）、数据、运行时特性状态。数据通过 `loadAll()`（Promise.allSettled）从后端拉取。所有数据操作为 async + 乐观更新 + IPC 调用 + 失败回滚。`persist()` 调用 setConfig 保存，浏览器模式 catch 静默降级。`hydrateFromConfig()` 从后端拉取配置。CONFIG_KEYS 覆盖 ui.*/input.*/service.*/feedback.* 键（反馈 GitHub Token 由后端 DPAPI 加密） |
...
| [windows/panel/tabs/ServiceTab.tsx](../src/windows/panel/tabs/ServiceTab.tsx) | ✅ | 服务配置页：ASR 提供商选择（auto/cloud/offline）、ASR/LLM 端点/密钥/模型配置、ASR 连接测试按钮（`testAsrConnection()`）、反馈通道配置（GitHub Issue Token + 邮件 JSON endpoint/收件人）、离线模型管理、翻译目标语言选择器 |
...
| [windows/panel/tabs/HelpTab.tsx](../src/windows/panel/tabs/HelpTab.tsx) | ✅ | 帮助页：版本 v0.1.0、快捷键说明、使用指南、AI 整理模式说明、项目主页链接、反馈表单弹窗（标题/描述/类型/联系方式、本地校验、提交中状态、成功/失败/离线缓存 Toast） |
...
| [lib.rs](../src-tauri/src/lib.rs) | ✅ | Tauri Builder setup：注册 `tauri-plugin-single-instance`、`autostart`、`updater`、`shell`、`process` → 初始化日志 → 隐藏/定位窗口 → 创建数据目录 → 打开 SQLite → 启动时自动 flush 离线反馈队列 → manage(Database/AppRuntime/PipelineHandle) → 启动反馈队列后台重试线程 → start_hotkey_pipeline → setup_tray → 注册 35 个 invoke_handler（新增 feedback 3 个）。**已注册**：autostart/single-instance/updater/shell/process 插件、系统托盘 |
...
| 文件 | 状态 | 说明 |
| :--- | :---: | :--- |
| [commands/mod.rs](../src-tauri/src/commands/mod.rs) | ✅ | 声明 audio/backup/config/dictionary/feedback/history/hotkey/model/preview/skills/updater 十一个子模块 |
| [commands/feedback.rs](../src-tauri/src/commands/feedback.rs) | ✅ | 反馈 IPC：`submit_feedback(input)` 校验后提交 GitHub Issue/邮件通道，网络失败写入离线队列；`flush_feedback_queue()` 重试队列；`list_feedback_queue()` 查询队列。启动时由 lib.rs 调用 `flush_feedback_queue_inner()` 自动重试一次，并启动后台线程每 120 秒重试离线队列 |
...
| 文件 | 状态 | 说明 |
| :--- | :---: | :--- |
| [services/mod.rs](../src-tauri/src/services/mod.rs) | ✅ | 声明 25 个子模块：app_context/asr/asr_cloud/asr_offline/audio/backup/clipboard/db/events/feedback/hotkey/injector/llm/logging/model_manager/paths/pipeline/preprocess/recorder/rewrite/secrets/skills/translate/tts/vad |
| [services/app_context.rs](../src-tauri/src/services/app_context.rs) | ✅ | 应用上下文识别：根据焦点窗口进程名识别当前应用（用于历史记录 `appContext` 字段） |
| [services/db.rs](../src-tauri/src/services/db.rs) | ✅ | SQLite 完整实现：Database(Connection) + open()/in_memory() + initialize()（WAL模式、foreign_keys、4张表：config/history/filter_words/feedback_queue）+ 历史/配置/过滤词 CRUD + 反馈离线队列 CRUD（enqueue/list/get/mark_attempt/delete）+ FeedbackQueueItem/NewFeedbackQueueItem + 单元测试 |
| [services/feedback.rs](../src-tauri/src/services/feedback.rs) | ✅ | 用户反馈服务：FeedbackInput/FeedbackType/FeedbackSubmitResult、输入清洗与长度/脚本协议校验、GitHub Issue 创建（GitHub REST API）、邮件 JSON POST 同步、HTTP 状态错误分类（可重试/致命）、队列项转回提交输入、默认 feedback.* 配置 |
...
| `services/secrets.rs` | ✅ | DPAPI 加密：`protect_secret()` / `unprotect_secret()` / `encode_config_value()` / `decode_config_value()`，自动加密 ASR/LLM API Key 与反馈 GitHub Token |
...
| `services/db.rs` | ✅ | SQLite：4 表（config/history/filter_words/feedback_queue）+ WAL + CRUD + 反馈离线队列 + 单元测试 |
